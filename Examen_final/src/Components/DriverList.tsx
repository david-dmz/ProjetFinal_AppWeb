...
<div className="text-center mb-10">
      <h2 className="text-4xl font-extrabold text-gray-900 tracking-tight mb-6">Grille des Pilotes</h2>
        ...
        {filteredDrivers.map((driver) => (
          ...
          >
            ...
            <div className="flex flex-col items-center p-6 flex-grow">
              ...
              <span className="text-sm font-semibold text-gray-400 mb-1">#{driver.driver_number}</span>
             ...
             {/* Le bouton change de style et de texte si le pilote est déjà favori */}
              <button 
                ...
                className={`mt-auto w-full inline-flex justify-center items-center px-4 py-2.5 text-sm font-bold text-white rounded-lg focus:ring-4 focus:outline-none transition-colors ${
                  favorites.some(fav => fav.driver_number === driver.driver_number)
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
              ...
              <div className="flex-1 w-full bg-gray-50 p-6 rounded-2xl border border-gray-100 space-y-4">
                <p className="flex justify-between items-center border-b border-gray-200 pb-2">
                  <strong className="text-gray-500 uppercase text-xs tracking-wider">Acronyme</strong> 
                  <span className="text-lg font-bold text-gray-900">{selectedDriver.name_acronym}</span>
                </p>
                <p className="flex justify-between items-center border-b border-gray-200 pb-2">
                  <strong className="text-gray-500 uppercase text-xs tracking-wider">Numéro</strong> 
                  <span className="text-lg font-bold text-blue-600">#{selectedDriver.driver_number}</span>
                </p>
                <p className="flex justify-between items-center border-b border-gray-200 pb-2">
                  <strong className="text-gray-500 uppercase text-xs tracking-wider">Écurie</strong> 
                  <span className="text-lg font-bold text-gray-900">{selectedDriver.team_name}</span>
                </p>
                <p className="flex justify-between items-center mb-6">
                  <strong className="text-gray-500 uppercase text-xs tracking-wider">Pays</strong> 
                  <span className="text-lg font-bold text-gray-900">{selectedDriver.country_code}</span>
                </p>
                
                {/* Bouton d'ajout aux favoris dans la modale */}
                <button 
                  className="w-full inline-flex justify-center items-center px-4 py-3 text-base font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 transition-transform active:scale-95"
                  onClick={(e) => { e.stopPropagation(); console.log("Ajout favori depuis la modale"); }}
                >
                   Ajouter aux Favoris
                </button>
              </div>
